...
public class EarthquakeCityMap extends PApplet {
	...
	PGraphics pg;
	Float x=(float) 50, y=(float)105,TRI_SIZE=(float)8,yk=(float)245;
	...
	public void setup() {		
		...
		/*for(Feature f: countries) {
			...
			for(String key:m.keySet()) {
				System.out.println(key+" "+m.get("coordinates"));
			}
			System.out.println();
		}*/
	    ...
	    for(PointFeature feature : earthquakes) {
		  ...
		  if(isLand(feature)) {
			 ...
			 /*  Marker mark=new LandQuakeMarker(feature);
	...
	private void addKey() {	
		...
		fill(color(128,0,0));
		triangle(x, y-TRI_SIZE, x-TRI_SIZE/2, y+TRI_SIZE/3, x+TRI_SIZE/2, y+TRI_SIZE/3);
		
		noFill();
		ellipse(50,125,8,8);
		rect(46,145,8,8);
		ellipse(50,245,8,8);
		line(x-SIZER, yk-SIZER, x+SIZER, yk+SIZER);
		line(x+SIZER, yk-SIZER, x-SIZER, yk+SIZER);
	...
	private void printQuakes() 
	{
		...
		int countChina=0,countOcean=0;
		for(Marker earthquake:quakeMarkers) {
			EarthquakeMarker em = (EarthquakeMarker)earthquake;
			System.out.print(em.getTitle()+" "+em.getMagnitude()+" "+em.getDepth()+" "+em.getRadius());
			if(em.isOnLand()) {System.out.print(" "+((LandQuakeMarker)em).getCountry() );
			if(((LandQuakeMarker)em).getCountry().equals("United States of America"))countChina++;}
			else countOcean++;
System.out.println();
		}
		System.out.println(countChina+" "+countOcean);
	}
